...
const double MAX_BOND_LENGTH_SQUARED = 10.00;  
...
double chain_chain_energy(double dist_sq) {
    ...
    const double barrier_sq = 13.0; // MAX_BOND_LENGTH_SQUARED
...
int main() {
   ...
   for (double T = T_START; T <= T_END + 0.0001; T += T_STEP) {
        ...
        long long int monte_carlo_sweep1 = 2e6 ;
        long long int M=0;
    ...
    save_results_to_csv(T_values, E_, Esquared_,Binder, c_1,c_1_fluct, c_2,c_2_fluct, c_3,c_3_fluct, Cv_, "results20quadraticpotentialcc1cw1_2D13.csv");
    return 0;
}