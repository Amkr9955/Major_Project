...
bool is_valid_bond_length(double dist_sq) {
    for (double allowed_dist_sq : allowed_bond_lengths_squared) {
        if (fabs(dist_sq - allowed_dist_sq) < 1e-3) { // Check if distance is one of the allowed values, 1e-6 for overflow control
            return true;
        }
    }
    return false;
...
double calculate_energy_change(const Monomer& current_monomer, const Monomer& other_chain_monomer, const Monomer& new_pos) {
    double energy_change = 0.00;

    // Chain to wall interaction
    if (current_monomer.z > WALL_DISTANCE && fabs(new_pos.z - WALL_DISTANCE) < 1e-3) {
        energy_change -= 1.00;  // Moving towards the wall
    } else if (fabs(current_monomer.z - WALL_DISTANCE) < 1e-6 && new_pos.z > WALL_DISTANCE) {
        energy_change += 1.00;  // Moving away from the wall
    }
    ...
    if (dist_sq > WALL_DISTANCE_SQUARED && fabs(new_dist_sq - WALL_DISTANCE_SQUARED) < 1e-3) {
    energy_change -= 1.00;  // Getting closer to the other chain's monomer
    } else if (fabs(dist_sq - WALL_DISTANCE_SQUARED) < 1e-6 && new_dist_sq > WALL_DISTANCE_SQUARED) {
    energy_change += 1.00;  // Moving farther from the other chain's monomer
     }
...
double calculate_total_energy(const vector<Monomer>& chain1, const vector<Monomer>& chain2) {
    ...
    for (int i = 0; i < CHAIN_LENGTH; ++i) {
        if (fabs(chain1[i].z - WALL_DISTANCE) < 1e-6) energy -= 1.00;
        if (fabs(chain2[i].z - WALL_DISTANCE) < 1e-6) energy -= 1.00;
        if (fabs(distance_squared(chain1[i], chain2[i]) - WALL_DISTANCE_SQUARED) < 1e-3) {
             energy -= 1.00;
        }
    }
...
double count_contacts(const vector<Monomer>& chain1, const vector<Monomer>& chain2) {
    ...
    for (int i = 0; i < CHAIN_LENGTH; ++i) {
        ...
        if (fabs(dist_sq - WALL_DISTANCE_SQUARED) < 1e-3) {
            contacts+=1.00;
        }
    }
    return contacts;
...
double count_contacts_wall(const vector<Monomer>& chain) {
    double contacts = 0;
    for (int i = 0; i < CHAIN_LENGTH; ++i) {
        if (fabs(chain[i].z - WALL_DISTANCE) < 1e-3) {
            contacts+=1.00;
        }
    }
    return contacts;
...
void average(vector<Monomer>& chain1, vector<Monomer>& chain2,double& total_energy,double& E,double& Esquared,double& c1,double& c1squared,double& c2,double& c2squared,double& c3,double& c3squared,double& BETA, long long int& sweep,long long int& M) {
...
void save_results_to_csv(const vector<double>& T_values, const vector<double>& E_, const vector<double>& Esquared_, const vector<double>& c_1,const vector<double>& c_1_fluct,const vector<double>& c_2,const vector<double>& c_2_fluct,const vector<double>& c_3,const vector<double>& c_3_fluct, const vector<double>& Cv_, const string& filename) {
    ...
    file << "Temperature,E,Esquared,C1,C1fluc,C2,C2fluc,C3,C3fluc,,Cv\n";
    for (size_t i = 0; i < T_values.size(); ++i) {
        file << T_values[i] << "," << E_[i] << "," << Esquared_[i] << "," << c_1[i] << "," << c_1_fluct[i] << "," << c_2[i] << "," << c_2_fluct[i] << "," << c_3[i] << "," << c_3_fluct[i] << "," << Cv_[i] << "\n";
    }
    file.close();
}
...
int main() {
    vector<double> E_, Esquared_, c_1,c_1_fluct,c_2,c_2_fluct,c_3,c_3_fluct, Cv_, T_values;
    ...
    double E = 0.00, Esquared = 0.00, c1 = 0.00,c1squa = 0.00,c2 = 0.00,c2squa = 0.00 ,c3 = 0.00,c3squa = 0.00,Cv=0.00,c1fluc=0.00,c2fluc=0.00,c3fluc=0.00;
   ...
   for (double T = T_START; T <= T_END + 0.0001; T += T_STEP) {
        ...
        long long int M=0;
            ...
            for (long long int sweep1 = 1; sweep1 <= monte_carlo_sweep1; sweep1++) {
                for (int step = 1; step <= monte_carlo_step; step++) {
                    average(chain1,chain2,total_energy,E,Esquared,c1,c1squa,c2,c2squa,c3,c3squa,BETA,sweep1,M);
                }
                
            }
       
        E = 0.00, Esquared = 0.00, c1 = 0.00,c1squa = 0.00,c2 = 0.00,c2squa = 0.00 ,c3 = 0.00,c3squa = 0.00,Cv=0.00,c1fluc=0.00,c2fluc=0.00,c3fluc=0.00;
        M=0;

        for (long long int sweep2 = 1; sweep2 <= monte_carlo_sweep1; sweep2++) {
            for (int step = 1; step <= monte_carlo_step; step++) {
                average(chain1,chain2,total_energy,E,Esquared,c1,c1squa,c2,c2squa,c3,c3squa,BETA,sweep2,M);
            }
        }
    ...
    save_results_to_csv(T_values, E_, Esquared_, c_1,c_1_fluct, c_2,c_2_fluct, c_3,c_3_fluct, Cv_, "results202D.csv");
    return 0;
}